...
<div class="cp-wrap">
  <div class="cp-box">
    ...
    <span class="material-icons cp-icon">lock_person</span>
    ...
    <div class="cp-form">
      ...
      <div class="cp-field">
        ...
        <div class="cp-input-wrap">
          <span class="material-icons cp-field-icon">lock_outline</span>
          <input [type]="mostrar1 ? 'text' : 'password'" [(ngModel)]="actual" placeholder="••••••••"/>
          <button class="cp-eye" type="button" (click)="mostrar1=!mostrar1">
            <span class="material-icons">{{ mostrar1 ? 'visibility_off' : 'visibility' }}</span>
          </button>
        </div>
      </div>
      ...
      <div class="cp-field">
        ...
        <div class="cp-input-wrap">
          <span class="material-icons cp-field-icon">key</span>
          <input [type]="mostrar2 ? 'text' : 'password'" [(ngModel)]="nueva" placeholder="Mínimo 8 caracteres"/>
          <button class="cp-eye" type="button" (click)="mostrar2=!mostrar2">
            <span class="material-icons">{{ mostrar2 ? 'visibility_off' : 'visibility' }}</span>
          </button>
        </div>
      </div>
      ...
      <div class="cp-field">
        ...
        <div class="cp-input-wrap">
          ...
          <button class="cp-eye" type="button" (click)="mostrar3=!mostrar3">
            <span class="material-icons">{{ mostrar3 ? 'visibility_off' : 'visibility' }}</span>
          </button>
        </div>
      </div>
      ...
      <div class="cp-reqs">
        <div class="cp-req" [class.ok]="nueva.length >= 8">
          <span class="material-icons">{{ nueva.length >= 8 ? 'check_circle' : 'radio_button_unchecked' }}</span>
          Mínimo 8 caracteres
        </div>
        <div class="cp-req" [class.ok]="nueva === confirmar && nueva.length > 0">
          <span class="material-icons">{{ nueva === confirmar && nueva.length > 0 ? 'check_circle' : 'radio_button_unchecked' }}</span>
...
.cp-icon { font-size:40px;color:#1e3a5f;margin-bottom:4px;opacity:.85; }

h2 { margin:0;font-size:22px;font-weight:700;color:#0f1f35; }
.cp-alerta { background:#fef3c7;border:1px solid #fde68a;border-radius:8px;padding:10px 14px;font-size:13px;color:#92400e;text-align:center;width:100%;box-sizing:border-box;display:flex;align-items:center;gap:6px;justify-content:center; }
...
.cp-field-icon { position:absolute;left:10px;font-size:18px;color:#94a3b8;pointer-events:none;line-height:1; }
.cp-input-wrap input { width:100%;padding:10px 40px 10px 36px;border:1.5px solid #dde3eb;border-radius:8px;font-size:14px;outline:none;background:#f8fafc;transition:border-color .15s;box-sizing:border-box; }
.cp-input-wrap input:focus { border-color:#1e3a5f;background:#fff; }

.cp-eye { position:absolute;right:8px;background:none;border:none;cursor:pointer;padding:4px;color:#94a3b8;display:flex;align-items:center;transition:color .15s; }
.cp-eye:hover { color:#1e3a5f; }
.cp-eye .material-icons { font-size:18px;line-height:1; }
...
.cp-req .material-icons { font-size:15px; }
.cp-req.ok { color:#16a34a; }

.cp-error { background:#fef2f2;border:1px solid #fecaca;border-radius:8px;padding:10px 14px;font-size:13px;color:#dc2626;width:100%;box-sizing:border-box;display:flex;align-items:center;gap:7px; }
.cp-error .material-icons { font-size:17px;flex-shrink:0; }
.cp-ok    { background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;padding:10px 14px;font-size:13px;color:#16a34a;width:100%;box-sizing:border-box;display:flex;align-items:center;gap:7px; }
.cp-ok .material-icons { font-size:17px;flex-shrink:0; }
