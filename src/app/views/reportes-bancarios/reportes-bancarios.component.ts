...
export class ReportesBancariosComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    this.http.get<any>(`${API}/bancos/meses`).subscribe({
      next: r => {
        ...
        for (const m of r.meses) {
          ...
          if (!vistos.has(key)) {
            vistos.add(key);
            lista.push({ mes: m.mes, anio: m.anio, label: `${MESES_CORTOS[m.mes.toLowerCase()] || m.mes} ${m.anio}` });
          }
        }
  ...
  procesar() {
    ...
    for (const tabla of TODAS_TABLAS) {
      ...
      for (const r of rows) {
        ...
        let categoria = 'egreso'; // default para egresos puros
        ...
        if (ingreso > 0) {
          ...
          // Ventas al crédito (exacto en clasificacion)
          else if (
            ['ventas al credito', 'ventas al crédito'].includes(clasi) ||
            sede.includes('©')
          ) {
            categoria = 'ventas_credito';
          }
          // Con ID POP → conciliado
          else if (idPop && idPop !== 'null' && idPop !== 'None' && idPop !== '') {
            categoria = 'ingresos_id';
          }
          // Sin ID POP, no prosegur, no ventas crédito → pendiente
          else {
            categoria = 'sin_id';
          }
        }
        ...
        allFilas.push({
          ...
          detalle: r.detalle || '',
          ingreso, egreso, id_pop: idPop,
          clasificacion: r.clasificacion || '',
          sede: r.sede || '',
          categoria,
          // conciliado = tiene id_pop, prosegur o ventas_credito
  ...
  calcularKpis() {
    ...
    // Solo contar ingresos (excluir egresos puros)
    const soloIngresos = d.filter(r => r.categoria !== 'egreso');
    const totalIngresos = d.reduce((s, r) => s + r.ingreso, 0);
    const totalEgresos = d.reduce((s, r) => s + r.egreso, 0);

    // Conciliados: con id_pop + prosegur + ventas_credito
    const conciliados = soloIngresos.filter(r => r.conciliado).length;
    // Pendientes: sin_id (ingresos sin identificar)
    const pendientes = soloIngresos.filter(r => r.categoria === 'sin_id').length;
  ...
  crearCharts() {
    ...
    if (this.chartCategoriaRef?.nativeElement) {
      const cats = ['sin_id', 'ingresos_id', 'prosegur', 'ventas_credito'];
      const labels = ['Pendientes', 'Con ID POP', 'Prosegur', 'Ventas Crédito'];
      const colores = ['#dc2626', '#16a34a', '#e6e204', '#0891b2'];
  ...
  get bancosDeEntidad(): string[] {
    if (this.filtroEntidad === 'todas') return TODAS_TABLAS;
